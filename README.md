...
> **Engineers:** Read the **[Engineering Overview →](https://tannermidd.github.io/foodtopia/engineering/)** for the architecture, database model, data flows, security boundaries, offline sync, and operations.
...
- **Browse the whole larder.** The full recipe catalog syncs to the device for offline browsing with live readiness tiers against the current inventory; household favorites and a cook history keep good meals one tap away. Missing ingredients from any recipe land on the shared shopping list in one tap, and the list syncs across every member's device.
- **Cooking that survives the kitchen.** Recipe evidence and the server cooking session persist on-device (Dexie), so closing the tab or restarting preserves the active cooking context.
