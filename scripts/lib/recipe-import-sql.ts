...
function validatePublicationRecipes(values: readonly Recipe[]): Recipe[] {
  if (values.length === 0) {
    throw new RecipeImportError("No public recipes were supplied for import.");
  }
  ...
  for (const recipe of recipes) {
    if (recipe.rights.status === "draft") {
      throw new RecipeImportError(
        `Recipe ${recipe.id} is a draft and cannot be imported as public content.`,
      );
    }
    if (recipe.rights.status === "reviewed") {
      if (
        ...
        throw new RecipeImportError(
          `Recipe ${recipe.id} is not genuinely reviewed: a nonblank reviewer and reviewedAt are required.`,
        );
      }
      ...
      if (recipe.rights.reviewer.length > 160) {
        throw new RecipeImportError(
          `Recipe ${recipe.id} reviewer metadata exceeds 160 characters.`,
        );
      }
    } else if (recipe.rights.reviewer !== null || recipe.rights.reviewedAt !== null) {
      throw new RecipeImportError(
        `Seeded recipe ${recipe.id} must not claim a reviewer or reviewedAt.`,
      );
    }
...
function recipeRow(recipe: Recipe): string {
  return [
    ...
    recipe.rights.reviewer === null ? "null" : sqlText(recipe.rights.reviewer),
    recipe.rights.reviewedAt === null
      ? "null::date"
      : `${sqlText(recipe.rights.reviewedAt)}::date`,
    `${sqlText(recipe.rights.status)}::public.recipe_review_status`,
    "null",
  ].join(", ");
}
...
export function generatePublicRecipeImportSql(
  values: readonly Recipe[],
): string {
  const recipes = validatePublicationRecipes(values);
